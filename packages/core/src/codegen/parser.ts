...
function parseOperation(
  ...
  const isPaginated = parameters.some(
    (p) => p.in === 'query' && PAGINATION_PARAMS.has(p.name),
  )
...
export function parseOpenApi(doc: unknown): SchemaAST {
  ...
  if ('swagger' in doc) {
    ...
    throw new SchemaParseError({
      message:
        `Swagger 2.x documents (swagger: ${version}) are not supported. ` +
        'Convert the spec to OpenAPI 3.x first — e.g. with the swagger2openapi ' +
        'tool or the editor.swagger.io "Convert to OpenAPI 3" action.',
  ...
  if (pathsRaw !== undefined) {
    for (const [path, pathItemValue] of Object.entries(pathsRaw)) {
      ...
      const inheritedParams = Array.isArray(pathItem['parameters'])
        ? pathItem['parameters']
        : []
