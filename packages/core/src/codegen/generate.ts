...
import { emitModules } from './moduleEmitter'
  ...
  try {
    ...
    if (isErrnoException(error) && error.code === 'ENOENT') {
      throw new ConfigurationError(
        `OpenAPI input file not found: "${input}". ` +
          'Check the path passed to the codegen (relative paths resolve from the ' +
...
export async function generate(options: GenerateOptions): Promise<GenerateResult> {
  ...
  return {
    ...
    sourceHash,
    files: ['api.types.ts', 'api.modules.ts', 'api.schema.hash', 'overrides/.gitkeep'],
  }
}
