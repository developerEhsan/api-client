 ...
 */
import type { OperationNode, SchemaAST, TypeNode } from '../types/openapi.types';
...
export function validateValue(
  ...
  switch (type.kind) {
    ...
    case 'primitive': {
      ...
      if (type.type === 'boolean' && typeof value !== 'boolean')
        return fail(path, 'boolean', value);
      if (type.enum && !type.enum.includes(value as string | number)) {
        return {
          valid: false,
          errors: [`${path}: ${JSON.stringify(value)} not in enum`],
        };
      }
      return OK;
    }
...
function fail(path: string, expected: string, value: unknown): ValidationResult {
  return {
    valid: false,
    errors: [`${path}: expected ${expected}, got ${describe(value)}`],
  };
}
...
export function responseTypeFor(op: OperationNode, status: number): TypeNode | undefined {
  return (
...
export function validateResponseBody(
  ...
  const operationId =
    ast.paths[pathTemplate]?.[method.toUpperCase() as keyof (typeof ast.paths)[string]];
