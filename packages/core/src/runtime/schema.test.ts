...
import { validateResponseBody, validateValue } from '../codegen/schemaValidator';
...
describe('validateValue', () => {
  ...
  it('validates a response body by path+method+status', () => {
    expect(
      validateResponseBody(ast, '/invoices/{id}', 'GET', 200, { id: 'a', amount: 5 }).valid,
    ).toBe(true);
...
describe('driftDetector', () => {
  ...
  it('throws in strict mode, warns in loose mode', () => {
    const docB = JSON.parse(JSON.stringify(doc)) as typeof doc;
    (docB.paths as Record<string, unknown>)['/invoices/{id}'] = undefined;
    const diff = diffSchemas(ast, parseOpenApi(docB));
