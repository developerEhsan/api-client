...
function operationSignature(op: OperationNode | undefined): unknown {
  ...
  return {
    ...
    body: op.requestBody
      ? { required: op.requestBody.required, type: typeSig(op.requestBody.type) }
      : null,
...
export function handleDrift(diff: SchemaDiff, policy: DriftPolicy): void {
  ...
  if (policy.mode === 'strict') {
    throw new SchemaError({
      message: `OpenAPI schema drift detected: +${diff.addedOperations.length} added, -${diff.removedOperations.length} removed, ~${diff.changedOperations.length} changed operations.`,
      diff,
    });
  }
