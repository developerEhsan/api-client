 ...
 */
export async function applyBearer(config: BearerAuthConfig): Promise<Partial<AuthContribution>> {
  ...
  if (mode === 'warn') {
    console.warn('[api-client] Bearer auth: no token available; sending unauthenticated.');
  }
  return {};
}
