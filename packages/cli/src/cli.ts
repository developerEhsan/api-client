 ...
 * (compare the current schema against the last generation).
 *
 * @example
 * # generate the client into ./src/generated
 * developerEhsan-api-client generate --input ./openapi.json --output ./src/generated
 *
 * @example
 * # pin the runtime base URL and re-generate on every change to the spec
 ...
 *
 * @example
 * # validate a spec (defaults to ./openapi.json when --input is omitted)
  ...
  try {
    switch (command) {
      ...
      case 'validate': {
        ...
        process.stdout.write(`[32m✔[0m Valid OpenAPI ${ast.openapiVersion} (${counts})\n`);
        break;
      }
      case 'diff': {
        const input = args.input ?? './openapi.json';
        ...
        if (!result.hashChanged) {
          process.stdout.write('[32m✔[0m No changes since last generation.\n');
        } else {
          process.stdout.write(
            `[33m±[0m Schema changed: +${result.addedOperations.length} -${result.removedOperations.length} ~${result.changedOperations.length} operations\n`,
          );
        }
        break;
      }
